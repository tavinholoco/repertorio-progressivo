...
[Download the latest APK](https://github.com/tavinholoco/repertorio-progressivo/releases/latest) · [Architecture](docs/architecture.md) · [Releases](https://github.com/tavinholoco/repertorio-progressivo/releases)
...
Continuous integration runs on GitHub Actions through [`.github/workflows/test.yml`](.github/workflows/test.yml), on every push to `master` and every pull request into it. The job type checks with `tsc --noEmit`, lints, and runs the full Jest suite on Node 20.

Work lands on `master` through pull requests, is verified by CI, and is tagged for each release.
